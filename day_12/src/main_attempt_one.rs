...
fn calculate_combinations(records: &Vec<usize>, groups: &Vec<usize>) -> (usize, HashSet<usize>) {
    ...
    for i in 0..2_usize.pow(amount_damaged as u32) {
        ...
        // println!("Gorups, records: {:?}, {:?}", groups, this_records);

        if groups_match_records_improved(&groups, &this_records) {
...
fn groups_match_records_improved(groups: &Vec<usize>, records: &Vec<usize>) -> bool {
    // println!(
    //     "{:?}",
    //     &records
    //         .split(|x| x == &1_usize)
